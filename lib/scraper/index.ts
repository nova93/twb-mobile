...
export default function scraper(data: string): HomeProps {
  const imageRegex = /<br><img src=".{4,20}"><br>/gim;
  const justImageRegex = /".*"/;
  const justHTMLRegex = /".*.html/;
  const prevRegex = /<a href=".{4,20}\.html"><img src="previous\.png"><\/a>/gim;
  const nextRegex = /<a href=".{4,20}\.html"><img src="next\.png"><\/a>/gim;
